...
const STANDARD_STAR3_PRICE_RATE = 1.3;
...
export function calcRecommendedStarPrices({
  ...
  // 道具鍛冶など、結晶情報を持たない通常商材。
  // ★3は原価の1.3倍を販売目安とし、★0〜2も原価基準で段階をつける。
  // これにより、結晶装備以外でも作成おすすめ率を計算できる。
  if (!crystalValues) {
    return {
      ...
      star3: roundMarketPrice(cost * STANDARD_STAR3_PRICE_RATE),
    };
  }
  ...
  if (!crystalEquipment) {
    // 結晶装備ではない防具・裁縫装備は、これまでどおりの計算を使う。
    return {
  ...
  if (shouldUseItemAxis(selectedSet, normalizedItems)) {
    ...
    normalizedItems.forEach((item, index) => {
      ...
      const fallbackLabel = isCraftToolSet(selectedSet)
        ...
        : `${slotLabel}${duplicateSlot ? duplicateIndex : ""}`;
      const itemName = String(item?.name ?? "").trim() || fallbackLabel;
      const displayLabel =
        isCraftToolSet(selectedSet) || duplicateSlot ? itemName : slotLabel;
